...
import { createRefreshToken } from '../../libs/token';
...
export async function loginHandler(req: Request, res: Response) {
    try {
        ...
        // FIXED: jwt.sign takes (payload, secret, options)
        const accessToken = jwt.sign(
            { sub: user.id, role: user.role, tokenVersion: user.tokenVersion },
            process.env.JWT_ACCESS_SECRET!,
            { expiresIn: '15m' }   // optional – if you want expiry; otherwise remove
        );
const refreshToken = createRefreshToken(String(user.id), user.role, process.env.JWT_REFRESH_SECRET!);        const isprod = process.env.NODE_ENV === 'production';
